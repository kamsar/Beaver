...
robocopy $SourceWebDirectory $WebWorkingDirectory /XA:HS /MIR /NDL /XD .git .svn aspnet_client /XF *.ai *.psd *.fla Thumbs.db

if($LASTEXITCODE -gt 7) {
    Write-Error "Robocopy threw an error. Aborting." -ErrorAction Stop
}